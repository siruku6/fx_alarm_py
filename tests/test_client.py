...
def test_failing_market_ordering(oanda_client):
    result = oanda_client.request_market_ordering(stoploss_price=None)
    assert 'error' in result


def test_market_ordering(oanda_client, dummy_market_order_response, dummy_stoploss_price):
    dummy_response = dummy_market_order_response
    with patch('oandapyV20.API.request', return_value=dummy_response):
        response = oanda_client.request_market_ordering('', dummy_stoploss_price)
...
def test_market_order_args(oanda_client, dummy_market_order_response, dummy_stoploss_price):
    data = {
        'order': {
            'stopLossOnFill': {'timeInForce': 'GTC', 'price': str(dummy_stoploss_price)[:7]},
            'instrument': oanda_client._OandaPyClient__instrument,
            'units': '-{}'.format(oanda_client._OandaPyClient__units),
            'type': 'MARKET',
    ...
    with patch('oandapyV20.endpoints.orders.OrderCreate') as mock:
        with patch('oandapyV20.API.request', return_value=dummy_response):
            oanda_client.request_market_ordering('-', dummy_stoploss_price)
...
def test_request_trailing_stoploss(oanda_client):
    result = oanda_client.request_trailing_stoploss()
    assert 'error' in result


def test_request_closing_position(oanda_client):
    result = oanda_client.request_closing_position()
    assert 'error' in result
