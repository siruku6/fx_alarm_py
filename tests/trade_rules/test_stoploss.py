...
class TestStepTrailing:
    ...
    @pytest.mark.parametrize("position_type, previous_low, previous_high, expected", PARAMETERS)
    def test_basic(self, position_type, previous_low, previous_high, expected, config):
...
class TestSupportOrResistance:
    ...
    @pytest.mark.parametrize("position_type, current_sup, current_regist, expected", PARAMETERS)
    def test_new_stoploss_price(self, position_type, current_sup, current_regist, expected):
        result = stoploss_strategy.support_or_registance(position_type, current_sup, current_regist)
        if result is None:
            assert result is expected
        else:
