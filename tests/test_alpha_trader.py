...
class TestGenerateEntryColumn:
    @pytest.fixture(name="committed_df", scope="module")
    ...
    def test_columns_adding(self, trader_instance: AlphaTrader, committed_df: pd.DataFrame):
        ...
        with patch("pandas.merge", return_value=candles):
            with patch(
                "src.lib.transition_loop.commit_positions_by_loop", return_value=committed_df
            ):  # as mock:
                result: pd.DataFrame = trader_instance._AlphaTrader__generate_entry_column(
                    candles, indicators
                )

        expected: pd.DataFrame = committed_df.rename(columns={"entryable_price": "entry_price"})
