...
def fixture_dummy_positions() -> pd.DataFrame:
    return pd.DataFrame(
        {
            "time": pd.date_range(start="2019/09/03", periods=12, freq="1H").astype(str),
            "position": (
                "buy_exit",
                None,
                "long",
...
class TestRun:
    def test_without_position(self, result_processor: ResultProcessor):
        ...
        pd.testing.assert_frame_equal(
            ...
            pd.DataFrame(
                [],
                columns=["time", "position", "entry_price", "possible_stoploss", "exitable_price"],
            ),
        )
    ...
    def test_with_positions(
        ...
        backtest_result: Dict[str, Union[str, pd.DataFrame]] = {
            "result": "",
            "candles": dummy_positions,
        }
